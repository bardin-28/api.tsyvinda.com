# Plan: S3 Uploads + K3d→K3s Infrastructure (INFRA-K3D-001 / INFRA-S3-001)

Merged plan for two linked workstreams: the K3d→K3s platform rollout and the
local-S3 (MiniStack) + backend S3 upload migration that rides on it.

---

# Part 1 — K3d → K3s Infrastructure
...
# Part 2 — Local S3 (MiniStack) + Backend S3 Uploads
